...
function SignUpPageContent() {
  ...
  const nameDescribedBy = useMemo(
    () => [errors.name ? 'signup-name-error' : null, errors.form ? 'signup-form-error' : null].filter(Boolean).join(' ') || undefined,
    [errors.form, errors.name]
  );
  const emailDescribedBy = useMemo(
    () => [errors.email ? 'signup-email-error' : null, errors.form ? 'signup-form-error' : null].filter(Boolean).join(' ') || undefined,
    [errors.email, errors.form]
  );
  const passwordDescribedBy = useMemo(
    () => [errors.password ? 'signup-password-error' : null, errors.form ? 'signup-form-error' : null].filter(Boolean).join(' ') || undefined,
  ...
  const handleEmailSignUp = async (event: React.FormEvent) => {
    ...
    if (!name.trim()) {
      nextErrors.name = 'Enter your full name so we can personalise your account.';
    }

    if (!email.trim()) {
      nextErrors.email = 'Enter the email address you want to use.';
    } else {
      try {
        sanitiseEmail(email);
      } catch {
    ...
    if (!password) {
      nextErrors.password = 'Create a password with at least 6 characters.';
    ...
    try {
      ...
      await signUpWithEmail(safeEmail, safePassword, safeName);
        ...
        <div className="bg-white rounded-lg border border-cgray-200 shadow-card p-8 w-full max-w-md">
          <div className="text-center mb-6">
            <Link href="/" className="inline-flex flex-col items-center justify-center hover:no-underline">
              ...
              <span className="text-sm text-cgray-600 mt-1">Sri Lanka&apos;s A/L learning platform</span>
            </Link>
          </div>

          <div className="text-center">
            <h1 className="text-2xl font-bold text-cgray-900 mb-1">Join ICONIC ACADEMY</h1>
            <p className="text-sm text-cgray-600 mb-6">
              {selectedPlan
          ...
          {errors.form ? (
            <p id="signup-form-error" role="alert" className="mb-4 rounded border border-cred-500 bg-cred-50 px-3 py-2 text-sm text-cred-500">
              {errors.form}
            </p>
          ) : null}
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-xs text-cgray-600 font-medium whitespace-nowrap">OR SIGN UP WITH EMAIL</span>
            <div className="flex-1 border-t border-cgray-200" />
          </div>
          ...
          <form onSubmit={handleEmailSignUp} className="space-y-4" noValidate>
            <div className="space-y-2">
              ...
              <div className="relative">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-cgray-500" />
                <Input
                  id="name"
                  name="name"
                  type="text"
                  ...
                  onChange={(event) => {
                    setName(sanitiseInput(event.target.value, { maxLength: 120 }));
              ...
              {errors.name ? (
                <p id="signup-name-error" role="alert" className="text-sm text-cred-500">
                  {errors.name}
                </p>
              ) : null}
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-cgray-500" />
                <Input
                  id="email"
                  name="email"
                  type="email"
                  ...
                  onChange={(event) => {
                    setEmail(sanitiseInput(event.target.value, { maxLength: 320, stripHtml: false }));
              ...
              {errors.email ? (
                <p id="signup-email-error" role="alert" className="text-sm text-cred-500">
                  {errors.email}
                </p>
              ) : null}
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-cgray-500" />
                <Input
                  id="password"
                  name="password"
                  ...
                  className="c-input h-auto pl-9 pr-16"
                  value={password}
                  aria-invalid={Boolean(errors.password)}
                  aria-describedby={passwordDescribedBy}
                  onChange={(event) => {
                    setPassword(sanitiseInput(event.target.value, { maxLength: 128, trim: false, stripHtml: false }));
                ...
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 rounded p-1 text-cgray-600 transition hover:bg-cgray-50 hover:text-cgray-900"
                >
                  {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
              {errors.password ? (
                <p id="signup-password-error" role="alert" className="text-sm text-cred-500">
                  {errors.password}
                </p>
              ) : null}
            </div>
